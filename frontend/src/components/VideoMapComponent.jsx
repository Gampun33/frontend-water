...
const WaterLevelOverlay = ({ percent, imageFrame, w = 77, h = 55 }) => {
  // พี่สาวคำนวณสัดส่วนน้ำข้างในให้สัมพันธ์กับขนาดกล่องที่ส่งมานะคะ
  const innerW = w * 0.83; // ประมาณ 83% ของความกว้างกล่อง
  const innerH = h * 0.65; // ประมาณ 65% ของความสูงกล่อง
  const bottomDist = h * 0.23; // ระยะห่างจากขอบล่าง
    ...
    >
      ...
      <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
        <span
          ...
          style={{ fontSize: `${w * 0.13}px` }} // ขนาดตัวเลขแปรผันตามความกว้างกล่อง
        >
          {Math.round(percent)}%
        </span>
      </div>
...
const STATION_OVERLAYS = {
  เขื่อนภูมิพล: { top: "22%", left: "18%", w: 120, h: 85 }, // เขื่อนใหญ่สะใจ
  เขื่อนกิ่วลม: { top: "35%", left: "79%", w: 77, h: 55 }, // ขนาดมาตรฐาน
  อ่างเก็บน้ำห้วยสมัย: { top: "37%", left: "48%", w: 60, h: 45 }, // อ่างจิ๋ว
};
...
const MAP_CAROUSEL_DATA = [
  ...
  },
  {
    ...
    video:
      "https://cdn.pixabay.com/video/2022/10/05/133690-757657935_large.mp4",
    image:
      "https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&q=80&w=2000&h=1125",
    markers: [],
  },
  {
    id: 3,
    region: "ภาคตะวันออกเฉียงเหนือ (Northeast)",
    video: "https://cdn.pixabay.com/video/2020/06/17/42221-432247076_large.mp4",
    image:
      "https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&q=80&w=2000&h=1125",
    markers: [],
  },
  {
    id: 4,
    ...
    video:
      "https://cdn.pixabay.com/video/2024/02/09/199958-911693639_large.mp4",
    image:
      "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&q=80&w=2000&h=1125",
    markers: [],
  },
  {
    id: 5,
    region: "ภาคตะวันออก (East)",
    video: "https://cdn.pixabay.com/video/2019/06/25/24855-344933999_large.mp4",
    image:
      "https://images.unsplash.com/photo-1505118380757-91f5f5632de0?auto=format&fit=crop&q=80&w=2000&h=1125",
    markers: [],
  },
  {
    id: 6,
    region: "ภาคใต้ (South)",
    video: "https://cdn.pixabay.com/video/2020/05/25/40133-424072619_large.mp4",
    image:
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&q=80&w=2000&h=1125",
    markers: [],
  },
];
...
const VideoMapComponent = ({ mode = "interactive", markers = [] }) => {
  ...
  const handlePrev = (e) => {
    ...
    setCurrentSlide(
      (prev) =>
        (prev - 1 + MAP_CAROUSEL_DATA.length) % MAP_CAROUSEL_DATA.length,
    );
    setTransform({ x: 0, y: 0, scale: 1 });
  };
  ...
  return (
    ...
    >
      <div
        className="w-full h-full relative origin-center transition-transform duration-300 ease-out will-change-transform"
        style={{
      ...
      >
        {mode === "report" ? (
          ...
          <video
            ...
            muted
            playsInline
            className="absolute inset-0 w-full h-full object-contain pointer-events-none opacity-100 transition-opacity duration-500 animate-fade-in"
          >
            ...
            markers.map((item, idx) => {
              ...
              return (
                <React.Fragment key={`dyn-${item.id}`}>
                  ...
                  {overlayPos && (
                    ...
                    >
                      <WaterLevelOverlay
                        percent={calcPercent}
                        imageFrame={damimage}
                        w={overlayPos.w} // 🟢 ส่งความกว้าง
                        h={overlayPos.h} // 🟢 ส่งความสูง
                      />
                    </div>
                  )}
                  ...
                  <div
                    className="absolute whitespace-nowrap text-center pointer-events-auto hover:scale-110 transition-all"
                    style={{
                      top: locConfig.set1.top,
                      left: locConfig.set1.left,
                      transform: "translate(-50%, -50%)",
                    }}
                  >
                    <span
                      className="font-bold drop-shadow-[0_2px_2px_rgba(0,0,0,0.9)]"
                      style={{ fontSize: "14px", color: customColor }}
                    >
                      {" "}
                      {/* 👈 ใส่สีตรงนี้ */}
                      {item.capacity
                  ...
                  <div
                    className="absolute whitespace-nowrap text-center pointer-events-auto hover:scale-110 transition-all"
                    style={{
                      top: locConfig.set2.top,
                      left: locConfig.set2.left,
                      transform: "translate(-50%, -50%)",
                    }}
                  >
                    <div
                      className="font-bold drop-shadow-[0_2px_2px_rgba(0,0,0,0.9)]"
                      style={{ fontSize: "12px", color: customColor }}
                    >
                      {" "}
                      {/* 👈 ใส่สีตรงนี้ด้วย */}
      ...
      <div className="absolute inset-0 pointer-events-none z-10">
        <button
          onClick={handlePrev}
          className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-black/30 hover:bg-black/60 text-white p-2 rounded-full pointer-events-auto print:hidden shadow-lg"
        >
          <ChevronLeft className="w-8 h-8" />
        </button>
        <button
          onClick={handleNext}
          className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-black/30 hover:bg-black/60 text-white p-2 rounded-full pointer-events-auto print:hidden shadow-lg"
        >
