         ...
         <table className="min-w-full divide-y divide-gray-200">
           <thead className="bg-gray-50">
             <tr>
               ...
               <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">สถานี</th>
               <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">ระดับน้ำ (ม.)</th>
           ...
           <tbody className="bg-white divide-y divide-gray-200">
             {myReports.length > 0 ? myReports.map((item) => (
               <tr key={item.id} className="hover:bg-gray-50 transition">
                 <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(item.updated_at || item.created_at).toLocaleTimeString('th-TH')}</td>
                 <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.stationName}</td>
                 <td className="px-6 py-4 whitespace-nowrap text-sm text-right font-mono">{item.waterLevel}</td>
                 <td className="px-6 py-4 whitespace-nowrap text-center"><StatusBadge status={item.status} /></td>
               </tr>
             )) : <tr><td colSpan="4" className="px-6 py-12 text-center text-gray-400"><FileText className="w-12 h-12 mb-3 mx-auto text-gray-300" /><p>ไม่พบประวัติการส่งข้อมูล</p></td></tr>}
           </tbody>
         </table>
       </div>
    </div>
